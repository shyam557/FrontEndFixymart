...
export default function AddPackageModal({ onClose, onAdd }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.title || !form.desc || !form.price || !form.services) return;
    onAdd({ ...form, price: Number(form.price), oldPrice: form.oldPrice ? Number(form.oldPrice) : undefined, services: Number(form.services), reviews: Number(form.reviews) });
    onClose();
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-purple-600 text-white py-2 rounded mt-2">Add Package</button>
        </form>
      </div>
    </div>
  );

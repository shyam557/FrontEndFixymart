...
export default function PainterServicesAdminPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // find Painter category
        const category = (cats || []).find((c) => {
          const name = (c.name || c.description || "").toString().toLowerCase();
          return name.includes("paint") || name.includes("painter") || name.includes("painting");
        });
        ...
        const filtered = (allServices || []).filter((svc) => {
          ...
          return (subId && subIds.has(subId)) || catName.includes("paint") || catName.includes("painter") || catName.includes("painting");
        });
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4">
      <div className="bg-white rounded-xl shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Painter — Services Management</h1>
            <p className="text-sm text-gray-500 mt-1">Manage painting services and packages</p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {services.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-3 py-6 text-center text-gray-500">No services found for Painter.</td>
                    </tr>
                  ) : (
                    services
                      .filter((s) => {

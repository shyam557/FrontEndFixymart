...
export default function ACServicesAdminPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // find AC / Appliance category (case-insensitive match)
        ...
        const acSubs = Array.isArray(acCategory?.subcategories)
          ? acCategory.subcategories.map((s) => ({ id: s.id, name: s.name }))
          : [];

        setSubcategories(acSubs);

        // filter services that belong to AC category/subcategories
        const acSubIds = new Set(acSubs.map((s) => String(s.id)));
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4">
      <div className="bg-white rounded-xl shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">AC & Appliance — Services Management</h1>
            <p className="text-sm text-gray-500 mt-1">Manage AC & Appliance services and packages</p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {services.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-3 py-6 text-center text-gray-500">No services found for AC & Appliance.</td>
                    </tr>
                  ) : (
                    services
                      .filter((s) => {
                          ...
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button onClick={async () => {
                              ...
                              try {
                                ...
                                const acSubIds = new Set(subcategories.map((sb) => String(sb.id)));

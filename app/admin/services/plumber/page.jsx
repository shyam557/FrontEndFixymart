...
export default function PlumberServicesAdminPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // find Plumber category (case-insensitive match)
        const plumberCategory = (cats || []).find((c) => {
          const name = (c.name || c.description || "").toString().toLowerCase();
          return name.includes("plumb") || name.includes("plumber");
        });

        const plumberSubs = Array.isArray(plumberCategory?.subcategories)
          ? plumberCategory.subcategories.map((s) => ({ id: s.id, name: s.name }))
          : [];

        setSubcategories(plumberSubs);

        // filter services that belong to Plumber category/subcategories
        const plumberSubIds = new Set(plumberSubs.map((s) => String(s.id)));
        const filtered = (allServices || []).filter((svc) => {
          ...
          // Also try category name match as fallback
          const catName = (svc.subcategory?.category?.name || svc.category || "").toString().toLowerCase();
          return (subId && plumberSubIds.has(subId)) || catName.includes("plumb") || catName.includes("plumber");
        });
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4">
      <div className="bg-white rounded-xl shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Plumber — Services Management</h1>
            <p className="text-sm text-gray-500 mt-1">Manage Plumber services and packages</p>
          </div>
          ...
          <div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  className="ml-2 flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded font-semibold"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {services.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-3 py-6 text-center text-gray-500">No services found for Plumber.</td>
                    </tr>
                  ) : (
                    services
                      .filter((s) => {
                            ...
                            <button onClick={async () => {
                              ...
                              try {
                                ...
                                setServices((all || []).filter((svc) => {
                                  const subId = svc.subcategory ? String(svc.subcategory.id ?? svc.subcategoryId ?? svc.subcategory) : null;
                                  const catName = (svc.subcategory?.category?.name || svc.category || "").toString().toLowerCase();
                                  return (subId && subIds.has(subId)) || catName.includes("plumb") || catName.includes("plumber");
                                }));
                              } catch (err) {
                                console.error(err);
          ...
          <div className="w-full max-w-2xl p-4">
            <AddServiceForm
              ...
              onSubmit={async () => {
                ...
                try {
                  const all = await fetchAllServices();
                  const acSubIds = new Set(subcategories.map((s) => String(s.id)));
                  setServices(
                    (all || []).filter((svc) => {
                      const subId = svc.subcategory ? String(svc.subcategory.id ?? svc.subcategoryId ?? svc.subcategory) : null;
                      const catName = (svc.subcategory?.category?.name || svc.category || "").toString().toLowerCase();
                      return (subId && acSubIds.has(subId)) || catName.includes("ac") || catName.includes("appliance");
                    })
                  );
                } catch (err) {
                  console.error(err);
                }
                // notify overview pages to refresh their lists
                try {
